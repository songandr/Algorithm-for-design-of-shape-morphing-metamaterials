function [cj, rij] = centerOfPanel3D(Pj, coord)
% Inputs:
% Pj: The set of all of the indices (as opposed to position) of y's within a panel
...
for i = 1:len
    sum = sum + coord(3*Pj(i)-2:3*Pj(i), 1);
end
...
rij = zeros(3*len, 1);
for i = 1:len
    rij(3*i-2:3*i, 1) = coord(3*Pj(i)-2:3*Pj(i), 1) - cj;
end
