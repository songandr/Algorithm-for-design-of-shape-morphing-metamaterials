# Algorithm for design of shape morphing metamaterials and effective membrane stiffness of origami and kirigami tessellations and tubes
Elastic energy minimization algorithm discussed in the manuscript "Algorithmic design and effective membrane stiffness of origami and kirigami tessellations and tubes" section 3.
...
1. Set up initial conditions for $\mathbf{x}\in\mathbb{R}^{2I}$, $\mathbf{y}\in\mathbb{R}^{3I}$, $\mathbf{R}\_j=I\in\mathbb{R}^{3x3}$, the rigidity constraint matrices $\mathbf{L}\_0$ and $\mathbf{L}$, and panel assignments $P\_j$.
...
4. Use plot4vectors3D for visualization within Mathematica. If a new topology (not Miura, Miura4x4, rotating squares, helical waterbomb), the necessary edge graphs must be written into the plot1vector, plot1vector3D functions beforehand.
...
Effective membrane stiffness (see section 4 of manuscript) can be calculated for any low energy solution through membraneStiffness.m given whether the pattern is planar (dim = 2) or not (dim = 3), and whether the group symmetries are dictated by translation or helical group operators. Helical structures must be initialized with $\mathbf{e}_2$ as the cylindrical axis with $\mathbf{y}_1 = r\mathbf{e}_3$.
