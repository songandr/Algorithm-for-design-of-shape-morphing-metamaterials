...
1. Set up initial conditions for x in 2D, y in 3D, R = Id in R^{3x3}, the rigidity constraint matrices L_0 and L, and panel assignments Pj. L must be ordered such that the first 3*|B_1| rows correspond to d1 and the next 3*|B_2| rows correspond to d2. The last row is reserved for chi_1 to fix the first node to the origin. Any redundant constraints must be removed to successfully run post-processing geometric stiffness calculations e.g. for an abelian group of isometries g_1 and g_2, if y_{i'} = g_1(g_2(y_i) = g_1(y_{i''}) = g_2(y_{i'''}), then only one of the constraints among y_{i'} = g_1(y_{i''}) and y_{i'} = g_2(y_{i'''}) should be embedded into L.
